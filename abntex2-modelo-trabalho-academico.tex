...
A detecção pode ser desafiadora devido à sua crescente sofisticação e disseminação rápida. Muitas vezes, essas notícias são projetadas para se assemelhar a informações reais, tornando difícil diferenciar. Além disso, as redes sociais e a rapidez da disseminação online podem amplificar essas notícias antes que correções ou possíveis verificações adequadas sejam feitas, aumentando ainda mais a dificuldade de identificação. Por outro lado, esta tarefa pode ser facilitada com o uso de tecnologias.
...
Deste modo, o trabalho apresenta uma heurística com o objetivo de determinar a combinação mais eficaz de técnicas de pré-processamento associada a um algoritmo de aprendizado treinado a partir de notícias falsas e verdadeiras, visando também a identificação de \textit{fake news} com um menor tempo de análise. Essas técnicas incluem a remoção de \textit{stop words} (palavras comuns sem grande relevância contextual), a \textit{stemização} (processo de redução de palavras à sua forma base) e a eliminação de caracteres especiais. Essas abordagens foram testadas individualmente e em grupos antes da aplicação dos algoritmos, e em seguida aplicadas aos algoritmos de Aprendizado de Máquina \textit{Naive Bayes}, \textit{Decision Tree} e \textit{Random Forest}, visando a detecção da veracidade do título da notícia.

Para a obtenção desses títulos destinados a alimentar a base de treinamento e teste, empregou-se o método de \textit{web scraping} \cite{glez2014web} em títulos de notícias previamente identificadas como falsas. Paralelamente, foram coletados, em quantidade equivalente, títulos de notícias verdadeiras utilizando a biblioteca \textit{Tweepy} na linguagem de programação \textit{Python}. As notícias falsas foram obtidas do E-Farsas\footnote{O \textbf{E-farsas} é um site criado em 1 de abril de 2002, pelo analista de sistemas Gilmar Lopes. Tem como objetivo desmentir boatos que circulam na internet. Disponível em: \nolinkurl{https://www.e-farsas.com/}}, que é um site que desempenha um papel fundamental na validação interna da veracidade das informações. Já as notícias verdadeiras foram extraídas do renomado veículo de comunicação G1\footnote{O \textbf{g1} é um portal de notícias brasileiro mantido pelo Grupo Globo e sob orientação da Central Globo de Jornalismo. Disponível em: \nolinkurl{https://twitter.com/g1}}. Esse processo de seleção das notícias é essencial para garantir a qualidade e a credibilidade das informações fornecidas para o algoritmo.

Para o algoritmo compreender os dados, é necessário converter o texto em valores numéricos, e para isso foi utilizado o \textit{CountVectorizer}. Segundo \citeonline{casimiro2020proposta}, \textit{CountVectorizer} é uma técnica que analisa a frequência de ocorrência de palavras em um conjunto de dados de texto. Ele trabalha criando uma matriz de contagem de palavras, onde cada linha representa um documento de texto e cada coluna representa uma palavra do vocabulário. Os valores nas células da matriz indicam quantas vezes cada palavra ocorre em cada documento. O \textit{CountVectorizer} é uma etapa crucial no pré-processamento de dados de texto, tornando os textos acessíveis e prontos para análise e modelagem.

O trabalho está organizado como se segue. No Capítulo 2 é realizada a fundamentação de toda a base teórica para entender o impacto de uma \textit{fake news}. Neste capítulo também é explicado a importância do pré-processamento do texto e como ela interage com a classificação automática. No Capítulo 3 é realizado um estudo sobre outros artigos que abordam o assunto de \textit{fake news}, IA e a associação de ambos. No Capítulo 4 são discutidos os materiais utilizados e a metodologia empregada no desenvolvimento do algoritmo. No Capítulo 5 é explicado passo-a-passo como foram realizados os experimentos do algoritmo classificação e os resultados com os algoritmos de pré-processamento escolhidos. Por fim, no Capítulo 6 é realizada a conclusão do trabalho e discussões acerca de trabalhos futuros.
% ---------------------------------------------------------------------------------------------
...
No contexto tecnológico atual, as \textit{fake news} representam uma ameaça em constante crescimento, impulsionada pela popularização da internet e das mídias sociais. Segundo \citeonline{costa2020classificador}, grupos com agendas políticas ou comerciais aproveitam-se dessas redes sociais para criar e espalhar informações falsas, aproveitando-se da natureza dinâmica e viral da tecnologia.
...
De acordo com \citeonline{de2020processamento}, PLN pode ser dividido em cinco estágios principais de análise. O primeiro estágio, a \textit{tokenização}, envolve a divisão do texto em unidades menores, como palavras. Em seguida, a análise léxica relaciona as formas das palavras às suas formas primitivas no dicionário. A análise sintática concentra-se nas relações entre as palavras em uma frase e como as frases se combinam em sentenças. A análise semântica busca extrair o significado das palavras, expressões e sentenças. Por último, a análise pragmática visa compreender o texto, observando referências pronominais e a coesão das frases adjacentes. Juntos, esses estágios permitem a extração computacional do significado pretendido pelo autor a partir de um documento de texto.
...
Em relação às \textit{fake news}, o PLN pode ser usado para identificar esse problema, analisando as informações das notícias. Os algoritmos podem ser treinados com grandes quantidades de dados previamente validados e, em seguida, usados para avaliar novas informações. Por exemplo, podem ser usados para identificar palavras ou frases que são comumente associadas a notícias falsas e detectar padrões na disseminação das \textit{fake news}.

\section{\textbf{Aprendizado de Máquina}}
O Aprendizado de Máquina (AM) concentra-se em desenvolver técnicas computacionais para facilitar o aprendizado, além de construir sistemas capazes de adquirir aprendizado de maneira automática. Um sistema de aprendizado se refere a um programa de computador que toma decisões com base em experiências acumuladas ao resolver com sucesso problemas anteriores \cite{monard2003conceitos}.

O campo de AM é considerada um ramo da área de IA, sendo uma área especializada no estudo e construção de sistemas que sejam capazes de aprender de forma automatizada a partir de dados \cite{brink2016real}. Em vez de serem explicitamente programadas para realizar tarefas específicas, os algoritmos de AM são treinados usando dados para reconhecer padrões e fazer previsões ou tomar decisões com base nesses padrões. Essa abordagem permite que os algoritmos melhorem sua precisão à medida que são expostas a mais dados, tornando-se especialmente úteis em tarefas complexas e com grandes conjuntos de dados.
...
1. \textbf{Aprendizado Supervisionado:} No Aprendizado Supervisionado, o modelo é treinado com um conjunto de dados que inclui exemplos de entrada e as correspondentes saídas desejadas. O objetivo é aprender a mapear as entradas para as saídas de maneira que o modelo possa fazer previsões ou classificações precisas em novos dados. É amplamente utilizado em problemas de classificação e regressão, onde o modelo pode prever categorias ou valores numéricos com base nos dados de treinamento.

2. \textbf{Aprendizado Não Supervisionado:} No Aprendizado Não Supervisionado, o modelo é treinado em um conjunto de dados que não contém saídas rotuladas. O objetivo principal é descobrir padrões ou estruturas nos dados, como grupos de dados semelhantes, sem orientação explícita. Isso é comumente usado em tarefas de clusterização, onde o modelo agrupa dados em \textit{clusters} com base em semelhanças.

3. \textbf{Aprendizado por Reforço:} O Aprendizado por Reforço envolve um agente que interage com um ambiente, toma ações e recebe recompensas com base nas ações tomadas. O agente aprende a tomar ações que maximizam a recompensa ao longo do tempo. Esse tipo de aprendizado é frequentemente aplicado em áreas como robótica e jogos, onde o agente aprende a otimizar seu comportamento com base nas recompensas obtidas.
...
Essa fórmula permite atualizar nossas crenças ou probabilidades com base em novas evidências. Ela é particularmente útil quando queremos avaliar a probabilidade de uma hipótese (\(A\)) ser verdadeira, dadas as evidências (\(B\)) que temos. A Fórmula de \textit{Bayes} é uma ferramenta poderosa para tomar decisões informadas em cenários onde a incerteza desempenha um papel significativo. \citeonline{durgiewicz2021detecccao} refere ao \textit{Naive Bayes} como um modelo simples de classificação, porém proporciona bons resultados mesmo quando comparados aos obtidos com classificadores mais complexos. 

\subsection{\textbf{\textit{Decision Tree}}}
\citeonline{charbuty2021classification} descreve a Árvore de Decisão como um método de Aprendizado de Máquina que constrói uma estrutura em forma de árvore para tomar decisões com base em condições e características. O algoritmo começa com um conjunto de dados e analisa as características para encontrar o atributo que melhor separa os dados em dois grupos. Em seguida, o algoritmo cria um nó para esse atributo e repete o processo para cada grupo. O processo continua até que todos os dados estejam separados em nós individuais. 

As Árvores de Decisão são conhecidas por sua capacidade de fornecer informações valiosas sobre como as decisões são tomadas em um determinado contexto. À medida que a árvore cresce e se ramifica, é possível visualizar facilmente quais características são mais importantes na classificação ou previsão, tornando-as uma ferramenta valiosa para análise de dados e tomada de decisões em ambientes empresariais e científicos.
...
Uma importante característica do algoritmo é sua capacidade de lidar com dados ausentes e valores atípicos. As árvores individuais podem ser construídas sem a necessidade de imputação de valores ausentes, o que economiza tempo e minimiza o risco de introduzir viés nos dados. O \textit{Random Forest} pode ser uma escolha confiável em cenários onde a qualidade dos dados pode ser variável e sujeita a perturbações. Sua flexibilidade, eficácia e capacidade de lidar com desafios comuns tornam-o um algoritmo essencial em aplicações de Aprendizado de Máquina e análise de dados.
...
No contexto de detecção de \textit{fake news}, a remoção de \textit{stop words} desempenha um papel importante. Ao eliminar palavras comuns, o algoritmo de detecção pode se concentrar nas palavras-chave e em padrões de linguagem mais distintos que são mais indicativos de notícias falsas. Isso pode ajudar a melhorar a precisão do modelo, pois as \textit{stop words} não trazem informações relevantes para a identificação de conteúdo enganoso. Portanto, a remoção de dessas palavras é uma estratégia valiosa para aprimorar os sistemas de detecção de \textit{fake news}, permitindo que eles identifiquem com maior precisão as características específicas das notícias falsas, como sensacionalismo, informações contraditórias e viés tendencioso, entre outros.

\subsection{\textbf{Remoção de Caracteres Especiais}}
A remoção de caracteres especiais é uma etapa importante de pré-processamento que visa a remoção de caracteres não alfa numéricos do texto, incluindo símbolos como \textit{hashtags}, arrobas, cifrões e outros caracteres especiais que não sejam letras ou números \cite{brandao2023impacto}, mantendo apenas os elementos essenciais para a análise. Com isso, pode ser útil para melhorar a eficácia de algoritmos de PLN e outras tarefas de processamento de texto. Segue alguns exemplos abaixo:
...
A \textit{stemização}, também conhecida como \textit{stemming} em inglês, é um processo linguístico que envolve a redução de palavras à sua forma base ou raiz, removendo afixos como sufixos e prefixos. Isso ajuda a tratar variações de uma mesma palavra, como tempos verbais diferentes, como uma única entidade. Por exemplo a palavra "correndo" vira \textit{corr}, mas podendo variar de acordo com o algoritmo aplicado.

Em resumo, os algoritmos de \textit{stemming} envolvem a normalização linguística, em que as diferentes formas de uma palavra são simplificadas para uma forma básica chamada "stem". O resultado da aplicação de algoritmos de \textit{stemming} é a eliminação de afixos (prefixos ou sufixos) de uma palavra, ou até mesmo a conversão de um verbo para sua forma infinitiva \cite{lovins1968development}.

\section{\textit{\textbf{Web Scraping}}}
O \textit{web scraping} é a prática de coleta de dados por meio do acesso direto a páginas \textit{web} que não envolve a interação de um programa com uma \textit{API} (Interface de Programação de Aplicação, em português) e que não é conduzida por um usuário humano utilizando um navegador \textit{web} \cite{glez2014web}. A técnica analisa o código \textit{HTML} das páginas \textit{web} e identifica os elementos de interesse, como textos, imagens ou \textit{links}, para posterior uso em análises.
...
\textit{Beautiful Soup} \cite{richardson2007beautiful} é uma biblioteca  \textit{Python} para extrair dados de arquivos \textit{HTML} e \textit{XML}. Essa biblioteca simplifica significativamente a análise de páginas da \textit{web}, tornando mais eficiente o acesso e a manipulação de informações contidas em documentos desse formato. Com ele, é possível buscar elementos específicos em uma página \textit{web}, extrair dados relevantes, como texto,  \textit{links} ou atributos, e navegar pela estrutura do documento de maneira simplificada.

Essa versatilidade faz dessa biblioteca uma ferramenta essencial em tarefas de \textit{web scraping}, mineração e análise de dados. Ao permitir que os desenvolvedores acessem e manipulem informações contidas em documentos \textit{HTML} com facilidade, o \textit{Beautiful Soup} contribui significativamente para projetos que envolvem a coleta de informações a partir de sites e páginas da internet. Com essa biblioteca, a tarefa de extrair dados relevantes de páginas \textit{web} torna-se mais eficaz e acessível, facilitando a obtenção de informações valiosas para análises, pesquisas e automação de tarefas.

\section{\textbf{\textit{Tweepy}}}
\textit{Tweepy} \cite{roesslein2009tweepy} é uma popular biblioteca \textit{Python} que simplifica a interação com a \textit{API} do \textit{X} (também conhecido popularmente como \textit{Twitter}), fornecendo métodos e classes convenientes para autenticação, consulta e manipulação de dados. Ela permite aos desenvolvedores acessar informações de usuários, seus \textit{tweets}, seguidores, entre outros, tornando-a uma escolha comum para tarefas de mineração de dados e análise de redes sociais. Além disso, o \textit{Tweepy} é valioso para a criação de \textit{bots} e automação de tarefas relacionadas ao \textit{Twitter}. Ao fornecer métodos simples para interagir com a \textit{API}, os desenvolvedores podem implementar respostas automáticas, monitoramento de atividades específicas e no contexto atual, extrair \textit{tweets} (postagens) de um determinado usuário.
...
Neste capítulo, abordaremos trabalhos previamente conduzidos que estão relacionados ao tema proposto. Esses trabalhos serviram como fundamentos para as pesquisas realizadas, contribuindo significativamente para uma compreensão aprofundada de vários conceitos e práticas dentro da área. Eles desempenharam um papel essencial na orientação de como lidar com as especificidades da base de dados utilizada, abrangendo aspectos como pré-processamento, extração de recursos e técnicas de PLN.

O trabalho de \citeonline{costa2020classificador} abordou o impacto social das \textit{fake news}, destacando a crescente pesquisa e interesse na detecção e categorização dessas notícias. Utilizando métodos de Aprendizado de Máquina, especificamente a técnica de classificação do classificador \textit{PassiveAgressiveClassifier} em conjunto com PLN, o objetivo foi classificar notícias como falsas ou verdadeiras. Quatro conjuntos de dados foram empregados, seguindo uma divisão de treino (70\%) e teste (30\%), sendo tratados com \textit{TfidfVectorizer} e submetidos ao classificador para análise de métricas como \textit{accuracy}, \textit{precision} e \textit{recall}. Todos os conjuntos alcançaram taxas superiores a 78\%, consideradas ótimas, com destaque para um determinado \textit{dataset}, que obteve uma taxa de 99\%. Esses resultados evidenciam o potencial do classificador e do PLN como alternativas eficazes na detecção de notícias falsas.

\citeonline{de2020processamento} apresenta pesquisas e estudos que abordam a detecção de notícias falsas e a disseminação dessas informações enganosas, considerando abordagens que envolvem a utilização de métodos de PLN e algoritmos de Aprendizado de Máquina. Esses estudos estabeleceram uma base sólida para o presente trabalho, que busca contribuir para a compreensão e enfrentamento das \textit{fake news}, reconhecendo a importância de abordagens abrangentes que considerem não apenas o conteúdo das notícias, mas também o contexto de sua disseminação nas redes sociais e outras plataformas online.

\citeonline{sakurai2019processamento} estuda técnicas de PLN e Aprendizado de Máquina. O trabalho utiliza uma base de dados composta por 3600 notícias, sendo 1800 verdadeiras e 1800 falsas, e foram extraídas 20 características de \textit{Part-of-Speech} e aplicadas em três diferentes algoritmos de Aprendizado de Máquina, o \textit{SVM}, \textit{Adaboost} e Rede Neural. O objetivo de \citeonline{sakurai2019processamento} é avaliar o desempenho desses algoritmos e correlacioná-los com o padrão de escrita presente nas notícias, e contribuiu bastante para o estudo atual.

O estudo atual apresenta uma abordagem distinta em relação aos estudos existentes sobre detecção de \textit{fake news}. Enquanto \citeonline{costa2020classificador} focou na aplicação do \textit{PassiveAgressiveClassifier} em conjunto com Processamento de Linguagem Natural (PLN) e \citeonline{de2020processamento} destacou a importância de abordagens abrangentes que considera o contexto de disseminação das notícias nas redes sociais, este trabalho se destaca por sua ênfase na busca por uma heurística específica para determinar o melhor conjunto de técnicas de pré-processamento de texto. A análise individual das combinações de algoritmos de pré-processamento e Aprendizado de Máquina, realizada de maneira sistemática, visa identificar as combinações que proporcionam a melhor média de acurácia e menor desvio padrão na detecção de \textit{fake news}. Dessa forma, este estudo contribui para o campo ao oferecer uma perspectiva mais focalizada na otimização das técnicas utilizadas, proporcionando uma base sólida para aprimoramentos futuros na detecção eficaz de notícias falsas.
% ---------------------------------------------------------------------------------------------
% Metodologia
% ---------------------------------------------------------------------------------------------
\chapter{Materiais e Métodos}
\label{chapter:materiaisemetodos}
O presente estudo adota uma abordagem de pesquisa experimental exploratória devido à necessidade de testar e validar diferentes combinações de técnicas de pré-processamento de texto e algoritmos de Aprendizado de Máquina. Utilizando a técnica de coleta de dados para análise de conteúdo, foram coletadas amostras de notícias verdadeiras e notícias falsas, a pesquisa aplicou diversas estratégias de pré-processamento. Quanto à técnica de análise de dados, a pesquisa empregou uma abordagem quantitativa ao utilizar diferentes algoritmos de aprendizado. Ao realizar análises estatísticas, a pesquisa busca não apenas teorizar, mas determinar as combinações mais eficazes para a detecção de \textit{fake news}, oferecendo uma base sólida para conclusões e avanços no campo da IA aplicada à verificação de informações.
...
Para a realização desta pesquisa, foi essencial conduzir a coleta de dados visando adquirir um conjunto representativo de títulos de notícias falsas e verdadeiras. Nesse sentido, foram selecionadas fontes de dados específicas, resultando na obtenção de 300 títulos de notícias falsas e 300 títulos de notícias verdadeiras. Desse total, 150 notícias verdadeiras e 150 notícias falsas foram alocadas igualmente em duas conjuntos distintos: um composto pela base de teste e outro pela base de treinamento.

A coleta dos títulos de notícias falsas envolveu um processo de \textit{web scraping} utilizando a biblioteca \textit{Beautiful Soup}, que foi aplicado ao site E-farsas. Foram selecionadas somente os títulos de notícias de categorias previamente verificadas como falsa pelo próprio site. Com isso temos as notícias falsas para a base de dados.

Os títulos de notícias verdadeiras foram extraídas do \textit{Twitter} do G1, utilizando da biblioteca \textit{Tweepy}. Os títulos foram consideradas como sendo verdade na base de dados, pois é um portal relevante e bem conhecido na internet. Por fim, juntamos essas notícias verdadeiras com as falsas, ambas já marcadas a sua veracidade e formamos a base de dados.

\section{\textbf{Arquitetura}}
Na etapa de vetorização dos dados de treinamento, empregou-se a técnica do \textit{CountVectorizer}, transformando textos em representações numéricas adequadas para o treinamento dos modelos de classificação. O \textit{CountVectorizer} permitiu a criação de uma matriz de contagem de palavras, onde cada coluna representa uma palavra única presente nos títulos das notícias e cada linha corresponde a uma instância de dados.

A técnica de validação cruzada foi realizada com 5 \textit{folds} (partições em que o conjunto de dados é dividido durante o processo de treinamento e avaliação). A escolha de \textit{k} é um equilíbrio entre variância e bias (tendência de um modelo em fazer previsões sistemáticas incorretas) na estimativa do desempenho do modelo. Um valor baixo de \textit{k} pode levar a uma estimativa instável e com alta variância, enquanto um valor muito alto de \textit{k} pode resultar em uma estimativa com bias maior, pois cada conjunto de treinamento é pequeno. O valor de \textit{k}=5 é uma escolha comum e frequentemente recomendada, pois fornece uma boa combinação de estabilidade na estimativa de desempenho do modelo e eficiência computacional.
...
A média de acurácia proporciona uma medida central que reflete o desempenho médio do modelo ao longo das várias execuções da validação cruzada. Essa métrica é calculada somando as acurácias obtidas e dividindo pelo número total. Uma média mais alta indica um melhor desempenho geral do modelo.

O desvio padrão é uma medida de dispersão que indica o quanto as acurácias individuais se desviam da média. Um desvio padrão menor sugere uma consistência maior nas predições do modelo. Isso é útil para entender a estabilidade e confiabilidade do modelo.

Ao analisar essas métricas para cada combinação de combinações, podemos obter uma compreensão abrangente do desempenho de cada modelo. Uma média de acurácia mais alta é desejada, indicando uma melhor capacidade de predição, enquanto um desvio padrão menor sugere uma maior consistência nas predições. Essas informações são cruciais para a seleção da combinação mais eficaz, considerando tanto a precisão média quanto a consistência nas predições.

Nesta sessão, serão apresentados os resultados dos experimentos realizados para avaliar o desempenho de diferentes combinações de pré-processamento em algoritmos de Aprendizado de Máquina. A análise se concentra em três algoritmos: \textit{Naive Bayes}, \textit{Decision Tree} e \textit{Random Forest}.

\section{\textbf{\textit{Naive Bayes}}}
Os resultados obtidos com o algoritmo \textit{Naive Bayes} revelam padrões interessantes em relação às diferentes combinações de pré-processamento aplicadas aos dados. Ao considerar a \textit{stemização}, a remoção de \textit{stop words} e a eliminação de caracteres especiais, observamos que a média da acurácia varia entre 82.67\% e 88.00\%. A menor variabilidade ocorre quando a \textit{stemização} é desativada e apenas a remoção de \textit{stop words} é realizada, indicando uma maior consistência nas predições. No entanto, a combinação que inclui todas as técnicas de pré-processamento apresenta a acurácia mais elevada, indicando uma eficácia significativa na melhoria do desempenho do \textit{Naive Bayes} na classificação dos dados.

É relevante notar que o desvio padrão, que representa a dispersão dos resultados, varia de 3.86\% a 6.20\%. Combinações que incluem a remoção de \textit{stop words} demonstram um desvio padrão relativamente mais alto, indicando uma maior variabilidade nos resultados. Esse fenômeno pode ser explorado em investigações futuras para compreender melhor a influência específica da remoção de \textit{stop words} nas predições do \textit{Naive Bayes}. Em geral, os resultados sugerem que a escolha cuidadosa das técnicas de pré-processamento pode impactar significativamente o desempenho do \textit{Naive Bayes} na tarefa de classificação.
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    C1 & 87.67\% & 73.59\% & 78.15\% \\
    C2 & 88.00\% & 73.91\% & 77.76\% \\
    C3 & 82.67\% & 75.69\% & 73.51\% \\
    C4 & 86.67\% & 71.20\% & 78.85\% \\
    C5 & 83.33\% & 71.60\% & 74.37\% \\
    C6 & 82.67\% & 75.71\% & 73.23\% \\
    C7 & 86.33\% & 71.44\% & 78.64\% \\
    C8 & 84.67\% & 70.10\% & 74.32\% \\
    \bottomrule
  \end{tabular}
  \caption{Resultados da média da acurácia para diferentes combinações de pré-processamento e algoritmos}
